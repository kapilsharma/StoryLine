...
import { hasFamilyFeatures } from "@shared/project";
...
function toForm(c: Character): FormState {
  return {
    ...
    mother: c.mother ?? "",
    spouse: c.spouse ?? [],
    custom: Object.entries(c.custom ?? {}).map(([key, value]) => ({
      key,
      value: String(value),
    })),
  };
...
export function CharacterForm({
  ...
  const toggleSpouse = (id: string): void =>
    set(
      "spouse",
      form.spouse.includes(id)
        ? form.spouse.filter((s) => s !== id)
        : [...form.spouse, id],
    );

  const dateInvalid = (value: string): boolean =>
    Boolean(value.trim() && !isPartialDate(value.trim()));
    ...
    <>
      ...
      {initial && form.name.trim() && form.name.trim() !== initial.name && (
        <p className="small muted">
          The file keeps its id (<code>{initial.id}</code>) when you rename —
          relations point at the id.{" "}
          <button
            className="link-btn"
      ...
      {/* The whole family block is meaningless when rows are topics or phases
          rather than people, so a general project hides it (#63). Values already
          on disk are preserved — `toCharacter` only writes fields the form holds,
          and the form is seeded from the saved character either way. */}
      {showFamily && (
        <div className="custom-fields">
          ...
          <p className="small muted">
            <span
              className="swatch"
              style={{
                background: colourFor(preview, familyColours),
                marginRight: 6,
              }}
            />
            Decides the colour on the tree, and the tree shows{" "}
            <strong>{displayName(preview) || "—"}</strong>. Left blank it is
            taken from the surname — set it explicitly for someone who kept
            their own surname after marrying in.
          </p>
          ...
          {dateInvalid(form.birthday) && (
            <p className="small error">Use YYYY, YYYY-MM or YYYY-MM-DD.</p>
          )}
          ...
          <div className="form-row">
            ...
            <select
              value=""
              onChange={(e) => e.target.value && toggleSpouse(e.target.value)}
            >
          ...
          <p className="small muted">
            Saved on both people, so the link never points one way only.
          </p>
